...
public:
    typedef size_t line_n;                    // 定义保存行号的类型别名（变为适用于StrVec的size_t）
    TextQuery(ifstream &);
...
TextQuery::TextQuery(ifstream &infile) : file(new StrVec)
{
    ...
    while (getline(infile, text)) {
        file->push_back(text);                       // 将每行文本保存到StrVect对象中
...
public:
    typedef size_t line_n;
    QueryResult() {}                         // 为了测试程序，定义一个默认构造函数
    QueryResult(const string &w, shared_ptr<StrVec> f,
                shared_ptr<set<line_n>> l) : 
        word(w), file(f), lines(l) { }
private:
    string word;
    shared_ptr<StrVec> file;                // 使用自定义StrVec类来保存文件内容
